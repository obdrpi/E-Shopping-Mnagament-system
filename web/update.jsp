<%-- 
    Document   : update
    Created on : 8 Apr, 2013, 11:19:32 PM
    Author     : dell
--%>
...
<html>
    ...
    <body>
               <%
               try{String user=request.getParameter("element_1");
...
PreparedStatement ps=con.prepareStatement("UPDATE customer SET USER_ID=?,PASSWORD=?,EMAIL_ID=?,F_NAME=?,M_NAME=?,L_NAME=?,GENDER=?,ADDRESS=?,MOBILE_NO=?");
...
if(!rs.next()||!rs1.next())
   ...
   if((user.isEmpty())||(email.isEmpty())||(pass.isEmpty())||(address.isEmpty()))
    a=1/0;
        ps.setString(1,user);
	ps.setString(2,pass);
