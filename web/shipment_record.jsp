...
<html>
    ...
    <body>
    <h1>RECORD STATUS</h1>
    <form action="shipment_java.jsp" method="POST">
...
ResultSet rs= st.executeQuery("SELECT * FROM record,BILL_PAYMENT where record.BILL_NO= bill_payment.BILL_NO ");%>
...
while(rs.next())
{
    ...
    <TR>
        ...
        <%if(rs.getString(2).equals("undelivered")){%>
        <TD><input type="text" name="<%=rs.getString(4)%>" value="<%=rs.getString(2)%>"></TD>
       <% }
               else{%>
        <TD><input type="text" name="" value="<%=rs.getString(2)%>" disabled="disabled"></TD>
        <%}%>
